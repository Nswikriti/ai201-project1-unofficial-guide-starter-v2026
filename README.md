...
The original indexing run produced chunks ranging from 178 to 549
characters, averaging 317 characters.

**Overlap:** 0 characters.

I chose to keep each campus post together because the documents are short
and contain related policy details. For example, the dining dollars post
explains both when funds roll over and when they expire. Separating these
sentences could remove an important condition from a chunk.

The original 800-character chunker already produced one chunk per document.
My strategy deliberately keeps each document intact rather than relying
on a fixed character limit. I expect the chunk count to remain 88.
This change alone does not demonstrate better retrieval.

My replacement function is `chunker.py::split_documents`. It removes
surrounding whitespace, skips empty documents, and preserves the source
filename. Longer documents covering multiple topics would need a
different strategy.
...
You can add a course through the end of the second week. Dropping is a longer window — through the end of week six — but a dropafter week two shows as a W on your transcript. Nothing anywhere on the registrar's site says this plainly, and students find out from each other.
